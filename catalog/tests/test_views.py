...
class AuthorCreateViewTest(TestCase):
    def setUp(self):
        ...
        permission = Permission.objects.get(name='Creat, update or delete authors')
        test_user2.user_permissions.add(permission)
        test_user2.save()
    ...
    def test_form_date_initially_euqal(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.get(reverse('author_create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_redirects_to_author_list_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        response = self.client.post(reverse('author_create'), {'first_name':'jarod', 'last_name': 'hao'})
        
        test_author = Author.objects.get(first_name = 'jarod')
        self.assertRedirects(response, reverse('author-detail', args=[str(test_author.pk)]))
